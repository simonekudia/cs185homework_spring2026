...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = torch.nn.Parameter(torch.tensor(np.log(temperature), dtype=torch.float32))
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # Note: Think about whether to use .rsample() or .sample() here...
        action = action_distribution.rsample()
        return -action_distribution.log_prob(action).mean(dim=-1)
        # ENDTODO
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # torch.zeros(batch_size, self.action_dim, device=obs.device) # replace this with the correct action
        ...
        q_values = self.critic(obs, action)
        # torch.zeros(self.num_critic_networks, batch_size, device=obs.device) # replace this with the correct q_values
        assert q_values.shape == (self.num_critic_networks, batch_size), q_values.shape
        # ENDTODO
        ...
        loss = -q_values.mean()
        # torch.tensor(0.0, device=obs.device) # replace this with the correct loss
        # ENDTODO
    ...
    def update(
        ...
        elif self.soft_target_update_rate is None:
            # hard update
