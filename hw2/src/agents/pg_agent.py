...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        T = len(rewards)
        discount_return = sum([self.gamma**t * rewards[t] for t in range(T)])
        return [discount_return] * T
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        T = len(rewards)
        discounted_rewards_to_go = np.zeros(T)

        for t in range(T):
            reward_to_go = sum([self.gamma**(tp - t) * rewards[tp] for tp in range(t, T)])
